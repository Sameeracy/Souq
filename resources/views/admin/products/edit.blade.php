<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <nav class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
            <a href="{{ route('admin.dashboard') }}" class="hover:text-brand-500 transition">Admin Dashboard</a>
            <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
            <span class="text-gray-800 dark:text-white font-medium truncate max-w-xs">Edit {{ $product->title }}</span>
        </nav>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 dark:border-gray-800 dark:bg-gray-dark shadow-theme-xs">
            <div class="border-b border-gray-100 dark:border-gray-800 pb-5 mb-6 flex items-center justify-between">
                <div>
                    <div class="inline-flex items-center space-x-2 px-2.5 py-0.5 rounded-full bg-purple-50 text-purple-700 dark:bg-purple-950/40 dark:text-purple-400 border border-purple-200 dark:border-purple-800 text-xs font-bold uppercase tracking-wider mb-2">
                        <span>Admin Product Override</span>
                    </div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Marketplace Product</h1>
                    <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">Listed by seller: <strong class="text-gray-800 dark:text-gray-200">{{ $product->seller->name ?? 'Unknown' }}</strong> ({{ $product->seller->email ?? 'N/A' }})</p>
                </div>
                <a href="{{ route('product.show', $product) }}" target="_blank" class="text-xs font-bold text-brand-500 hover:text-brand-600">
                    View Live &rarr;
                </a>
            </div>
            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" 
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3">
                            @if($product->image_path)
                                <div class="w-11 h-11 rounded-lg bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0 border border-gray-200 dark:border-gray-700">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" required
                              class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description', $product->description) }}</textarea>
                </div>
                ...
                <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                    <a href="{{ route('admin.dashboard') }}" class="px-5 py-2.5 border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium text-sm rounded-lg transition">
                        Cancel
                    </a>
