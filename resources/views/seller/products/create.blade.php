<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <nav class="flex items-center space-x-2 text-sm text-slate-500 mb-6">
            <a href="{{ route('seller.dashboard') }}" class="hover:text-indigo-600 transition">Seller Dashboard</a>
            ...
            <span class="text-slate-800 font-semibold">Add New Product</span>
        </nav>
        ...
        <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-10">
            <div class="border-b border-slate-100 pb-6 mb-8">
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">List a New Product</h1>
                <p class="text-sm text-slate-500 mt-1">Provide product details, price, images, and customizable variants with individual pricing</p>
            </div>
            ...
            <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="file" name="image" id="image" accept="image/*"
                               class="w-full bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" required
                              ...
                              class="w-full bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-3.5">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="bg-slate-50 rounded-2xl p-5 sm:p-6 border border-slate-200/80">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <h3 class="text-sm font-bold text-slate-900">Customizable Variants & Custom Pricing (Optional)</h3>
                            <p class="text-xs text-slate-500 mt-0.5">Specify option names, values, and an optional variant-specific price (Rs.)</p>
                        </div>
                    ...
                    <div class="space-y-3 mt-4">
                        ...
                        <div x-show="options.length === 0" class="text-xs text-slate-400 italic text-center py-2">
                            No variants configured. Product will be listed at base price only.
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4 flex items-center justify-end space-x-3">
                    <a href="{{ route('seller.dashboard') }}" class="px-5 py-3 text-sm font-semibold text-slate-600 hover:text-slate-900 transition">
                        Cancel
                    </a>
                    <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-xl shadow-lg shadow-indigo-600/30 transition">
                        Publish Product
                    </button>
                </div>
            </form>
